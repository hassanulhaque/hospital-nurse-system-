# HOSPITAL MANAGEMENT SYSTEM (THIS APP IS FOR THE NURSES TO KEEP TRACK OF NEW PATIENTS ENTRIES, MEDICANS CHECKOUTS ETC.... )
...
@app.route('/adduser')
def adduser():
    new_user = users("Hassan","123456")
    db.session.add(new_user)
    new_user = users("Neel","123456")
    db.session.add(new_user)
    new_user = users("Narender","123456")
    db.session.add(new_user)
    new_user = users("Jayesh","123456")
    db.session.add(new_user)
    new_user = users("Ritesh","123456")
    db.session.add(new_user)
    db.session.commit()
